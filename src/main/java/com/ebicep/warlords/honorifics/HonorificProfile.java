...
public class HonorificProfile {
    ...
    private final Set<Honorific> unlockedHonorifics = new HashSet<>();
    private final Set<HonorificColor> unlockedColors = new HashSet<>();
    private final Set<HonorificFont> unlockedFonts = new HashSet<>();
    ...
    public synchronized Document toDocument(UUID uuid) {
        ensureDefaults();
        return new Document("_id", uuid.toString())
                .append("unlockedHonorifics", enumNames(unlockedHonorifics))
    ...
    private synchronized void ensureDefaults() {
        ...
        if (!unlockedColors.contains(selectedColor)) {
            selectedColor = HonorificColor.AQUA;
        }
        if (!unlockedFonts.contains(selectedFont)) {
