...
public class StatsLeaderboardCTF extends AbstractStatsLeaderboardGameType<
        DatabaseGameCTF,
        DatabaseGamePlayerCTF,
        CTFStats,
        StatsLeaderboardCategory<DatabaseGameCTF, DatabaseGamePlayerCTF, CTFStats>> {
...
//        add(new StatsLeaderboardCategory<>(DatabasePlayer::getCtfStats, "All Queues", "All"));
    ...
    public void addExtraLeaderboards(StatsLeaderboardCategory<DatabaseGameCTF, DatabaseGamePlayerCTF, CTFStats> statsLeaderboardCategory) {
        ...
        statsLeaderboards.add(new StatsLeaderboard("Flags Captured",
                StatsLeaderboardLocations.LEAD_5,
                databasePlayer -> statsLeaderboardCategory.getStatFunction().apply(databasePlayer).getFlagsCaptured(),
                databasePlayer -> NumberFormat.addCommaAndRound(statsLeaderboardCategory.getStatFunction().apply(databasePlayer).getFlagsCaptured())
        ));
        statsLeaderboards.add(new StatsLeaderboard("Flags Returned",
                StatsLeaderboardLocations.CIRCULAR_1_OUTER_2,
                databasePlayer -> statsLeaderboardCategory.getStatFunction().apply(databasePlayer).getFlagsReturned(),
                databasePlayer -> NumberFormat.addCommaAndRound(statsLeaderboardCategory.getStatFunction().apply(databasePlayer).getFlagsReturned())
        ));
    }
