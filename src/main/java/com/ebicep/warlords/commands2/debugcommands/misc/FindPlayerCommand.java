...
public class FindPlayerCommand extends BaseCommand {
    ...
    public void findPlayer(CommandIssuer issuer, @Conditions("requireGame") @Flags("other") Player player) {
        ...
        ChatChannelCommand.sendDebugMessage(issuer, ChatColor.GREEN + "Found player " + ChatColor.RED + player.getName() + (isSpectator ? ChatColor.GREEN + " (Spectating)" : "(Playing)") + ChatColor.GREEN + " in game " + ChatColor.RED + game.getGameId());
