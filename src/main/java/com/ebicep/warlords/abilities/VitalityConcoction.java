    ...
    protected boolean onActivateInternal(@Nonnull WarlordsEntity wp) {
        ...
        if (pveMasterUpgrade2) {
            wp.doOnStaticAbility(ImpalingStrike.class, impalingStrike -> {
                modifiers.add(impalingStrike.getEnergyCost().addModifier(FloatModifiable.ModifierType.ADDITIVE_MULTIPLIER, "Concoction Party", -.75f));
                    }
            );
        }
        ...
        cooldown.addModifier(Modifier.MODIFY_INCOMING_DAMAGE_AFTER_INTERVENE, (event, currentDamageValue) -> {
            currentDamageValue.addModifier(FloatModifiable.ModifierType.MULTIPLICATIVE_MULTIPLIER, name, convertToDivisionDecimal(damageResistance));
                }
        );
        ...
        if (pveMasterUpgrade) {
            for (WarlordsEntity we : PlayerFilterGeneric
                ...
                we.getCooldownManager().addCooldown(new RegularCooldown<>(
                    ...
                    currentDamageValue.addModifier(FloatModifiable.ModifierType.MULTIPLICATIVE_MULTIPLIER, name, convertToDivisionDecimal(damageResistance / 2f));
                        }
                ));
