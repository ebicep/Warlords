            ...
            private void quarterStep(boolean last) {
                ...
                if (shouldExplode) {
                    ...
                    for (WarlordsPlayer p : PlayerFilter
                        ...
                        Vector v;
                        if (p == directHit) {
                            v = player.getLocation().toVector().subtract(p.getLocation().toVector()).normalize().multiply(-0.96).setY(0.2);
                        } else {
                            v = entity.getLocation().toVector().subtract(newLoc.toVector()).normalize().multiply(0.96).setY(0.2);
                        }
